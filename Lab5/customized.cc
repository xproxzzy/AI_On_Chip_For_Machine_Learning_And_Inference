...
/////////////////////////////////////////////////////////////////
    ...
    for (int out_c = 0; out_c < filter_N; out_c += 8) {
        ...
        // height loop
        for (int j = 0; j < (padded_h - filter_H + 1); j += tiled_stride_h) {

            // weight loop
            for (int i = 0; i < (padded_w - filter_W + 1); i += tiled_stride_w) {
                uint64_t dram_addr;

                int tiled_w = (i<=(padded_w-tiled_size))?tiled_size:(padded_w-i); // width you tiled to transfer ifmap from DRAM to SRAM
                int tiled_h = (j<=(padded_h-tiled_size))?tiled_size:(padded_h-j); // height you tiled to transfer ifmap from DRAM to SRAM
                int padding_t = (j==0)?1:0; // top padding or not(1 for padding)
                int padding_l = (i==0)?1:0; // left padding or not(1 for padding)
                int padding_r = (i>=(padded_w-tiled_size))?1:0; // right padding or not(1 for padding)
                int padding_b = (j>=(padded_h-tiled_size))?1:0; // bottom padding or not(1 for padding)
                ...
                for (int k = 0; k < ifmap_C; k += 8) {
                    ...
                    int ifmap_i = (i==0)?i:(i - padding_L);
                    int ifmap_j = (j==0)?j:(j - padding_T);
                    dram_addr = DRAM_INPUT_START_ADDR + 
                                ifmap_W * ifmap_H * k +
                                ifmap_i + ifmap_j * ifmap_W;
                ...
                dram_addr = DRAM_OUTPUT_START_ADDR + 
                            (padded_w - filter_W + 1) * (padded_h - filter_H + 1) * out_c +
                            i + j * (padded_w - filter_W + 1);
                encode_tranfer_config(
